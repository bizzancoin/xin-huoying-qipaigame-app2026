...
以下是源码目录中可识别的代表性模块。原版与二开目录不重复计算，具体可交付版本需结合演示确认。
...
目录存在表示项目中包含相关工程，不等于所有模块均已完成编译和运行验收。

## 产品画面

以下图片由发布方提供，展示游戏界面与开发工程。图片中的历史品牌、网址、账号、金额及活动文案仅属于原画面，不作为本项目的联系方式、售价或服务承诺；对应交付版本以演示确认为准。
...
5. **按约验收。** 按约定环境核对文件、编译结果与演示流程。

## 开发环境与项目结构

客户端包含 Cocos2d-x / Lua 工程；服务端使用 C++ / Visual Studio；Web 使用 .NET Framework，不同子项目可见 4.5、4.5.1、4.8 等目标版本。购买前应按交付版本确认完整工具链、数据库版本与依赖。
...
以上为源码项目结构说明。公开展示页面不代表上述源码目录可免费下载。

## 购买前常见问题

**这是免费或开源项目吗？**  
不是。本项目源码付费销售，公开展示不授予完整源码的免费使用或分发权。

**可以二次开发吗？**  
面向有二次开发需求的团队提供源码交付咨询，具体可修改模块和使用授权在购买前确认。

**购买后可以直接运行吗？**  
需要匹配对应版本的编译环境、数据库和配置。本文未对全部游戏及目标平台进行完整运行验收，是否提供编译成品和部署协助应写入交付清单。

**价格多少？包括维护吗？**  
完整源码价格为 **500 美元（USD 500）**，包含**所有前后端原始代码**，包括游戏客户端、服务端、Web 前台及管理后台。部署、定制、维护及升级不包含，如何收费，购买前单独约定。
...
本项目按商业方式付费交付。源码、配套资源的使用范围，以及修改、转售、再分发等权限，以双方明确约定为准；第三方组件与素材遵循各自的许可要求。
